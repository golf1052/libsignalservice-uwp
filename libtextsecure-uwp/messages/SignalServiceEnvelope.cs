...
namespace libtextsecure.messages
{
    ...
    public class SignalServiceEnvelope
    {
        ...
        public SignalServiceEnvelope(byte[] ciphertext, string signalingKey)//throws InvalidVersionException, IOException
        {
            ...
            envelope = Envelope.ParseFrom(getPlaintext(ciphertext, cipherKey));
        }
            ...
            string relay, long timestamp, byte[] legacyMessage, byte[] content)
        {
            Envelope.Builder builder = Envelope.CreateBuilder()
                                       ...
                                       .SetSource(source)
                                       .SetSourceDevice((uint)sourceDevice)
                                       .SetRelay(relay)
        ...
        private byte[] getPlaintext2(byte[] ciphertext, byte[] cipherKey)
        {
            byte[] ivBytes = new byte[IV_LENGTH];
            Array.Copy(ciphertext, IV_OFFSET, ivBytes, 0, ivBytes.Length);
            IBuffer iv = ivBytes.AsBuffer();

            SymmetricKeyAlgorithmProvider cipher = SymmetricKeyAlgorithmProvider.OpenAlgorithm(SymmetricAlgorithmNames.AesCbcPkcs7);
            CryptographicKey key = cipher.CreateSymmetricKey(cipherKey.AsBuffer());

            byte[] message = new byte[ciphertext.Length - VERSION_LENGTH - IV_LENGTH - MAC_SIZE];
            Array.Copy(ciphertext, CIPHERTEXT_OFFSET, message, 0, message.Length);

            return CryptographicEngine.Decrypt(key, message.AsBuffer(), iv).ToArray();
        }
